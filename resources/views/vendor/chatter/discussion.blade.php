...
@section(Config::get('chatter.yields.head'))
    ...
    @elseif($chatter_editor == 'trumbowyg')
        <link href="{{asset('public/vendor/devdojo/chatter/assets/vendor/trumbowyg/ui/trumbowyg.css')}}" rel="stylesheet">
        <style>
...
@if( Request::is( Config::get('chatter.routes.home')) )
    Forum
@elseif( Request::is( Config::get('chatter.routes.home') . '/' . Config::get('chatter.routes.category') . '/*' ) && isset( $discussions[0]->category->name ) )
    {{ $discussions[0]->category->name }}
...
<div id="chatter" class="discussion">
	...
	<div class="container margin-top">
	    <div class="row">
	       <div class="col-md-9">
	       	  <ol class="breadcrumb bg-none">
	  		    ...
	  		    <li><a href="{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $discussion->category->slug }}">{{ $discussion->category->name }}</a></li>
	  		    @if(isset($discussion->title))<li class>{{ $discussion->title }}</li>@endif
    	      </ol>
	       </div>
        	
	    	<div class="forum-search col-md-3">
	    	  <form role="form" method="POST" action="">
	    	    <div class="input-group">
	    	        <input type="text" class="form-control" name="forum-search" placeholder="Search">
        ...
        <div class="col-md-9 right-column">
            <h1 class="thread-title">{{ ucfirst($discussion->title) }}</h1>
            <div class="thread-details">
                <span class=""> <i class="fa fa-user"></i> by <a href="{{URL::to('/')}}/{{ \DevDojo\Chatter\Helpers\ChatterHelper::userLink($posts[0]->user) }}" > {{ ucfirst($posts[0]->user->name) }}</a></span>
				<span class="post-date">
					<i class="icon-calendar"></i>
					<span class="month"> {{ date('d M Y',strtotime($posts[0]->created_at)) }}</span>
				</span>
				<span class="comments"> <i class="fa fa-comments"></i> <a href="#"> {{ count($posts)-1 }} comment(s)</a></span>
			</div>
	                		...
	                		<span class="chatter_posts">
		                		...
		                		<div class="chatter_avatar">
				        			@if(Config::get('chatter.user.avatar_image_database_field'))
				        				...
				        				@else
				        					<img src="{{ URL::to('/') }}/{{ Config::get('chatter.user.relative_url_to_image_assets') . $post->user->{$db_field}  }}">
				        				@endif
				        		...
				        		<div class="chatter_middle">
				        			<span class="chatter_middle_details"><a href="{{URL::to('/')}}/{{ \DevDojo\Chatter\Helpers\ChatterHelper::userLink($post->user) }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a> <span class="ago chatter_middle_details">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
            	...
            	<div id="new_response">
            		...
            		<div class="chatter_avatar">
	        			@if(Config::get('chatter.user.avatar_image_database_field'))
	        				...
	        				@else
	        					<img src="{{URL::to('/')}}/{{ Config::get('chatter.user.relative_url_to_image_assets') . Auth::user()->{$db_field}  }}">
	        				@endif
		            ...
		            <div id="new_discussion">
			            ...
			            <form id="chatter_form_editor" action="{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}/posts" method="POST">
					...
					<div id="discussion_response_email">
						...
						@if(Config::get('chatter.email.enabled'))
							<div id="notify_email">
								<img src="{{asset('public/vendor/devdojo/chatter/assets/images/email.gif')}}" class="chatter_email_loader">
			...
			@else
				...
				<div id="login_or_register">
					<p>Please <a href="{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}/login">login</a> or <a href="{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}/register">register</a> to leave a response.</p>
				</div>
			...
			<div class="col-md-3 left-column">
			  ...
			  <div class="chatter_sidebar">
			      <button class="new-thread" id="new_discussion_btn"><span><i class="fa fa-plus-circle"></i></span> New {{ Config::get('chatter.titles.discussion') }}</button>
			      <h4><i class="fa fa-comments"></i> All categories</h4>
			      <div class="sepa"></div>
			          <ul class="nav nav-pills nav-stacked">
			            <?php $categories = DevDojo\Chatter\Models\Models::category()->all(); ?>
			            @foreach($categories as $category)
			                <li><a href="{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $category->slug }}"><div class="chatter-box" style="background-color:{{ $category->color }}"></div> {{ ucfirst($category->name) }}</a></li>
			            @endforeach
			          </ul>
			        <!-- /.box-body -->
			      </div>
    ...
    @if(Config::get('chatter.sidebar_in_discussion_view'))
        <div id="new_discussion_in_discussion_view">
            ...
            <form id="chatter_form_editor_in_discussion_view" action="{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}" method="POST">
                <div class="row">
                    <div class="col-md-7">
                        <!-- TITLE -->
                ...
                <div id="new_discussion_footer">
                    ...
                    <a href="{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}" class="btn btn-default pull-right" id="cancel_discussion">Cancel</a>
...
@elseif($chatter_editor == 'trumbowyg')
    ...
    <script src="{{asset('public/vendor/devdojo/chatter/assets/vendor/trumbowyg/trumbowyg.min.js')}}"></script>
    <script src="{{asset('public/vendor/devdojo/chatter/assets/vendor/trumbowyg/plugins/preformatted/trumbowyg.preformatted.min.js')}}"></script>
    <script src="{{asset('public/vendor/devdojo/chatter/assets/js/trumbowyg.js')}}"></script>
@endif
...
<script>
	$('document').ready(function(){
		...
		$('.discussions li').on('click', '.update_chatter_edit', function(e){
			...
			$.form('{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}/posts/' + post_id, { _token: '{{ csrf_token() }}', _method: 'PATCH', 'body' : update_body }, 'POST').submit();
		});
		...
		$('.delete_response').click(function(){
			...
			$.form('{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}/posts/' + post_id, { _token: '{{ csrf_token() }}', _method: 'DELETE'}, 'POST').submit();
		});
        ...
        @if(Config::get('chatter.sidebar_in_discussion_view'))
            ...
            $('#new_discussion_btn, #cancel_discussion').click(function(){
                @if(Auth::guest())
                    window.location.href = "{{route('homepage')}}/{{ Config::get('chatter.routes.home') }}/login";
                @else
